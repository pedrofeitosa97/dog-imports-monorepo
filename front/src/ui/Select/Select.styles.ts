...
export const SelectWrapper = styled.div<{ $hasError?: boolean; disabled?: boolean; $dark?: boolean }>`
  position: relative;
  display: flex;
  align-items: center;
...
export const StyledSelect = styled.select<{ $dark?: boolean }>`
  flex: 1;
...
export const ChevronIcon = styled.span<{ $dark?: boolean }>`
  position: absolute;
